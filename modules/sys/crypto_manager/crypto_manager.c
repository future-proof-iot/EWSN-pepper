                       ...
                       uint8_t* secret, size_t secret_len)
{
    ...
    ret = wc_curve25519_import_private_ex(sk->sk, CURVE25519_KEYSIZE, &sec, EC25519_LITTLE_ENDIAN);
    if (ret) {
        goto exit;
    }
    ret = wc_curve25519_import_public_ex(pk->pk, CURVE25519_KEYSIZE, &pub, EC25519_LITTLE_ENDIAN);
    if (ret) {
        goto exit;
    }
    ret = wc_curve25519_shared_secret_ex(&pub, &pub, secret, &secret_len, EC25519_LITTLE_ENDIAN);
